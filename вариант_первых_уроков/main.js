const products = [
    ...
    {id: 6, title: 'USB-card', price: 37},
];

const randerProduct = (productttt) => {
    return   `<li class="products__item">
                    <div class="products__item-wrapper">
                            <img class="products__item-img" src="./img/iii.png" alt="product-1 " width="360 " height="420 "> 
                    ...
                    <div class="products__item-text">
                        <h3 class="products__subtitle">${productttt.title}</h3>
                        ...
                        <p class="products__price">$${productttt.price}</p>
                    </div>
            </li>`
};
...
const randerProductPage = list => {
    document.querySelector('.products__list').innerHTML = list.map(item => randerProduct(item)).join('');
};


//тоже самое более подробно
/* const randerProductPage = list => {
    const productsList = list.map(item => randerProduct(item.title, item.price));
    let result = (productsList.join(''));
    document.querySelector('.products__list').innerHTML = result;
}; */


randerProductPage(products);
...
/* const renderCart = (product) => {
        return  `<div class="cart__card" >
                        ...
                        <div class="cart__order-details-box">
                            <div class="cart__order-details-title">${product.title}</div>
                            <div class="cart__order-details-point">price: <span class="cart__pink-price">${product.price} </span></div>
                            <div class="cart__order-details-point cart__order-details-quantity">quantity:</div>
                        </div>
                </div>`
}; */

// Вариант 1 более подробно 
/*  const randerCartPage = list => {
    const cartList = list.map(item => renderCart(item.title, item.price));
    let result = (cartList.join(''));
    document.querySelector('.proudct').innerHTML = result;
}; */
 // Вариант 2 запись отпимизированна
/* const randerCartPage = list => {
document.querySelector('.proudct').innerHTML = list.map(item => renderCart(item)).join('');
}; */
 
//randerCartPage(products);


let string = 'kgfsdfgsdjfgh'
string.endsWith
